...
def get_controlled_vehicles(route_filename, connection_info, \
    num_controlled_vehicles=10, num_uncontrolled_vehicles=0, pattern = 1):
    '''
...
def test_dijkstra_policy(vehicles):
    ...
    run_simulation(scheduler, vehicles,1)


def test_random_policy(vehicles, avg_deadline):
    print("Testing Random's Algorithm Route Controller")
    scheduler = RandomPolicy(init_connection_info)
    run_simulation(scheduler, vehicles, avg_deadline)


def run_simulation(scheduler, vehicles, avg_deadline):
    simulation = StrSumo(scheduler, init_connection_info, vehicles, avg_deadline)
...
if __name__ == "__main__":
    ...
    vehicles = get_controlled_vehicles(route_file, init_connection_info, 10, 30)
    ...
    for vid, v in vehicles.items():
        ...
        total_deadline += v.deadline
    avg_deadline = total_deadline/len(vehicles.items())
    test_dijkstra_policy(vehicles)
    test_random_policy(vehicles, avg_deadline)